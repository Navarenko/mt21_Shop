...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			$data['text_aviable'] = $this->language->get('text_aviable');
			$data['text_no_aviable'] = $this->language->get('text_no_aviable');

			$data['entry_qty'] = $this->language->get('entry_qty');
			$data['entry_qty_pcs'] = $this->language->get('entry_qty_pcs');
			...
			$data['cur_cours'] = $this->language->get('cur_cours');
			$data['text_special_prise'] = $this->language->get('text_special_prise');
			...
			$data['groupid'] = $this->customer->getGroupId();						//MY VAR
			...
			if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
				$data['price'] = $this->currency->format($this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax')), 0, 0, 1, 0); //for round price in my place
			} else {
				$data['price'] = false;
			}
				...
				foreach ($option['product_option_value'] as $option_value) {
					if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
						if ((($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) && (float)$option_value['price']) {
							$price = $this->currency->format($this->tax->calculate($option_value['price'], $product_info['tax_class_id'], $this->config->get('config_tax') ? 'P' : false), 0, 0, 1, 0);
						} else {
							$price = false;
						}
			...
			$data['attribute_groups'] = $this->model_catalog_product->getProductAttributes($this->request->get['product_id']);
                ...
                if (isset($product_info['base_price']) and $product_info['base_price'] > 0) {
                    $data['base_price'] =  $this->currency->format(1, $product_info['base_currency_code'], $product_info['base_price'], true);
					 $data['int_base_price'] =  $product_info['base_price'];
                }
                else {
                    $data['base_price'] = '';
                }
			...
			foreach ($results as $result) {
				...
				if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
					$price = $this->currency->format($this->tax->calculate($result['price'], $result['tax_class_id'], $this->config->get('config_tax')), 0, 0, 1, 0);
				} else {
					$price = false;
				}
			...
			$data['html_visibility_no_available_text'] = is_my_region() ? "" : "display_none";
		...
		if ($product_info && $recurring_info) {
			if (!$json) {
				...
				if ($recurring_info['trial_status'] == 1) {
					$price = $this->currency->format($this->tax->calculate($recurring_info['trial_price'] * $quantity, $product_info['tax_class_id'], $this->config->get('config_tax')), 0, 0, 1, 0);
					$trial_text = sprintf($this->language->get('text_trial_description'), $price, $recurring_info['trial_cycle'], $frequencies[$recurring_info['trial_frequency']], $recurring_info['trial_duration']) . ' ';
				} else {
					$trial_text = '';
				}

				$price = $this->currency->format($this->tax->calculate($recurring_info['price'] * $quantity, $product_info['tax_class_id'], $this->config->get('config_tax')), 0, 0, 1, 0);

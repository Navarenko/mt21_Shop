        ...
        <div class="table-responsive">
          <table class="table table-bordered">
			...
			foreach ($import_items->Sklad->item as $item) {
				if ($item->ref != "") {
					if (substr_count($item->ref, $product['model']) || substr_count($product['model'], $item->ref)) {
			...
			foreach ($import_items->Sklad->item as $item) {
				if ($item->ref != "") {
					if (substr_count($item->ref, $product['model']) || substr_count($product['model'], $item->ref)) {
						$stock_status = $stock_status + $item->Ostatok;
						$flag = true;
						break;
					}
				}
			}
			$flag_individual_items = substr_count("22904 22915 22957 22959 22960 24423 24487 24488 24606", $product['model']);
			if ($flag || $flag_individual_items || ($product['model'] == "набор")) {
				if (($stock_status - $product['quantity'] > 1) || ($product['model'] == "набор")) {
					echo '<span class="available_text">'. $text_aviable .'</span>';
				} elseif (($stock_status - $product['quantity'] <= 1) || ($flag_individual_items)) {

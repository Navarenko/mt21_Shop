...
<!--        ДОСТАВКА НА 1 ШАГЕ    из payment_method    -->

<script type="text/javascript">
$('input[name=\'shipping_method\']').on('change', function() {
	if (this.value == 'pickup') {
		$('#block-payment-addess').hide();
	} else {
		$('#block-payment-addess').show();
	}
	if (this.value != 'shipping') {
		$('#block_time_date_shipping').hide();
	} else {
		$('#block_time_date_shipping').show();
	}
});
</script>

<div class="radio">
    <label class="no_display">
      <input type="radio" name="shipping_method" value="deafult" checked="checked" />
      <?php echo $text_choose_method_shipping; ?></label>
    <label>
      <input type="radio" name="shipping_method" value="pickup" />
      <?php echo $text_pickup; ?></label>
  </div>
 <div class="radio">
    <label>
      <input type="radio" name="shipping_method" value="shipping" />
      <?php echo $text_shipping; ?></label>
  </div>
 <div class="radio">
    <label>
      <input type="radio" name="shipping_method" value="transport" />
      <?php echo $text_transport; ?></label>
  </div>
  <br>

<!--       АДРЕС    -->
  <div id="block-payment-addess" style="display: <?php echo ($addresses ? 'none' : 'block'); ?>;">
	<!-- Дата и время доставки -->
	<div id="block_time_date_shipping">
		<div class="block_date_shipping">
			<p>Дата доставки:</p>
			<input type="text" data-modal="true" data-lock="from" data-default-date='<?php $tomorrow  = mktime(0, 0, 0, date("m")  , date("d")+1, date("Y")); echo date("m-d-Y", $tomorrow); ?>' data-large-mode="true" data-lang="ru" data-format="d, F" name="date_ship" id="input-date_ship" data-disabled-days="05/01/<?php echo date('Y'); ?>,05/08/<?php echo date('Y'); ?>,05/09/<?php echo date('Y'); ?>" class="form-control">
             <script>
                $('#input-date_ship').dateDropper();
            </script>

			<!-- <script type="text/javascript">
			  $(function () {
				$('#input-date_ship').datetimepicker(
				  {minDate:"<?php
					$tomorrow = mktime(0, 0, 0, date("m")  , date("d")+1, date("Y"));
					echo date('d/m/Y', $tomorrow); 
					?>", daysOfWeekDisabled: [0, 6], pickTime: false, language: 'ru'}
				);
			  });
			</script> -->
		</div>
		<div class="block_time_shipping">
			<p>Время доставки:</p>
			<div id="time-existing">
				<select name="time_ship" class="form-control">
					<option selected="selected"></option>
					<option>с 10:00 до 14:00</option>
					<option>с 14:00 до 18:00</option>
				</select>
			</div>
		</div>
    </div>

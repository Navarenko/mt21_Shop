...
<?php if ($kassa_mode){ ?>
	<form method="POST" action="<?php echo $kassa_action; ?>" id='paymentForm' <?php  
			$file = 'payment_method.txt';
			$current = file_get_contents($file);
			if ($current == 'mymodule') echo 'class="no_display"'; ?>>
			<h3><?php echo $method_label; ?></h3>	
			<table class="radio">
				<tbody>
					<?php if ($method_cards && $current == 'mymodule'){?>
						<tr class="highlight">
							<td><input type="radio" name="paymentType" value="AC" checked id="ym2"></td>
							<td><label for="ym2"><?php echo $method_cards_text;?></label></td>
						</tr>
					<?php } ?>
					<?php if ($method_ym){?>
						<tr class="highlight">
							<td><input type="radio" name="paymentType" value="PC" <?php if ($current == 'yamodule') echo 'checked'; ?> id="ym1"></td>

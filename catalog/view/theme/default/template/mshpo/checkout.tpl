...
<?php if ($error_die) { ?>
	<div class="errortext"><ul><li><?php echo $error_die; ?></li></ul></div>
<?php } else { ?>
...
<style type="text/css">
    ...
    #collapse-payment-address .form-horizontal .checkbox, .form-horizontal .radio {
        margin-bottom: 10px;
    }
    #collapse-payment-method .panel-body .radio {
        margin-bottom: 20px;
    }
    div#collapse-payment-address .form-horizontal .pull-right,
    div#collapse-payment-method .pull-right,
    div#collapse-checkout-confirm .pull-right {
        width: 100%;
        margin-left: -15px;
        position: absolute;
    }
    div#collapse-payment-address .form-horizontal .pull-right input,
    div#collapse-payment-method .pull-right input,
  ...
  <div class="row">
    <div id="content" class="<?php echo $class; ?>">
      <h1><?php echo $heading_title; ?></h1>
		...
		<?php if (count($arMSHPErrors)>0) { ?>
			<div class="errortext">
				<ul>
					<?php foreach($arMSHPErrors as $error){ ?>
						<li><? echo $error; ?></li>
					<?php } ?>	
				</ul>
			</div>				
...
<script>//$('#content').prepend(222)</script>
...
$(document).ready(function() {
    $.ajax({
        ...
        success: function(html) {
			...
			$("#collapse-payment-address .panel-body").find("input[name='shipping_method'][value='deafult']").removeAttr("checked");
			$("#collapse-payment-address .panel-body").find("input[name='shipping_method'][value='pickup']").click().attr("checked", "checked").prop("checked", true);
    ...
    $.ajax({
        ...
        success: function(json) {
            ...
            if (json['redirect']) {
                ...
                $.ajax({
                    url: 'index.php?route=mshpo/confirm',
                    dataType: 'html',
                    complete: function() {

<?php
class ControllerMshpoPaymentMymodule extends Controller
{
	...
	public function index() {
		...
		$this->language->load('payment/mymodule');
		$data['button_confirm'] = $this->language->get('button_confirm');
		$data['p2p_action'] = $this->url->link('mshpo/payment/yamodule/yaredirect', '', 'SSL');
		...
		$data['shopSuccessURL'] = $this->url->link('mshpo/createOrder/success', '', 'SSL');
		$data['shopFailURL'] = $this->url->link('mshpo/createOrder/failure', '', 'SSL');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/mymodule.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/mymodule.tpl', $data);
		} else {
			return $this->load->view('default/template/payment/mymodule.tpl', $data);
		}
	}
	...
	public function makeOrder($order_id, $red = true)
	{
		if ($order_id)
		{
			...
			if ($red)
				$this->response->redirect($this->url->link('mshpo/createOrder/success', '', 'SSL'));
		}
	}
	...
	public function yaredirect()
	{
		if (isset($_POST['payment-type']) && !empty($_POST['payment-type']))
		{
			if ($_POST['payment-type'] == 'wallet')
			{
				...
				$auth_url = $this->buildObtainTokenUrl($this->config->get('ya_p2p_idapp'), $this->url->link('mshpo/payment/yamodule/insidewallet', '', 'SSL'), $scope);
			...
			elseif ($_POST['payment-type'] == 'card')
			{
				...
				$this->response->redirect($this->url->link('mshpo/createOrder', '', 'SSL'));
		}
		else
			$this->response->redirect($this->url->link('mshpo/createOrder', '', 'SSL'));
	}
			...
			if($response->status == 'success')
			{
				...
				do{
					$process_options = array(
						...
						'instance_id' => $instance_id,
						'ext_auth_success_uri' => $this->url->link('mshpo/payment/yamodule/successcard', '', 'SSL'),
						'ext_auth_fail_uri' => $this->url->link('mshpo/createOrder/failure', '', 'SSL')
					);	
	...
	public function insidewallet()
	{
		...
		if (empty($code))
			$this->response->redirect($this->url->link('mshpo/payment/yamodule/yaredirect', '', 'SSL'));
		else
		{
			$response = $this->getAccessToken($this->config->get('ya_p2p_idapp'), $code, $this->url->link('mshpo/payment/yamodule/yaredirect', '', 'SSL'), $this->config->get('ya_p2p_pw'));
			$error = '';
			$token = '';
				...
				switch($request_payment->status)
				{
					...
					case 'refused':
						...
						if($this->config->get('ya_p2p_log'))
							...
							$this->response->redirect($this->url->link('mshpo/createOrder/failure', '', 'SSL'));
						break;

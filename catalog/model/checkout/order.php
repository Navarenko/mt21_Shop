...
class ModelCheckoutOrder extends Model {
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				/* 'payment_date_ship'       => $order_query->row['payment_date_ship'], */
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				if (substr_count($order_info['payment_method'], "On-line")) {
					$data['text_greeting'] = sprintf($language->get('text_new_greeting_online_payment'), $order_info['store_name']);
				} else {
				...
				if ($order_info['payment_address_format']) {
					...
					if ($this->session->data['payment_address']['method_ship'] == "Курьером") {
						if ($this->session->data['payment_address']['date_ship']) {
							$format .= '{date_ship}' . "\n";
							$this->session->data['payment_address']['date_ship'] = "Дата доставки: " . $this->session->data['payment_address']['date_ship'];
						} 
						if($this->session->data['payment_address']['time_ship']) {
							$format .= '{time_ship}' . "\n";
							$this->session->data['payment_address']['time_ship'] = "Временной интервал доставки: " . $this->session->data['payment_address']['time_ship'];
						}
					}
					$format .= '{address_1}' . "\n" . '{address_2}' . "\n" . '{city} {postcode}' . "\n" . '{zone}' . "\n" . '{country}';
				}
				...
				$replace = array(
					'firstname' => 'ФИО: '.$order_info['payment_firstname'],
					'lastname'  => $order_info['payment_lastname'],
					'company'   => 'Компания: '.$order_info['payment_company'],
					'method_ship' => 'Доставка: ' . $this->session->data['payment_address']['method_ship'],
					'date_ship' => $this->session->data['payment_address']['date_ship'],
					'time_ship' => $this->session->data['payment_address']['time_ship'],
					'address_1' => 'Адрес: '.$order_info['payment_address_1'],
					'address_2' => $order_info['payment_address_2'],
					'city'      => 'Город: '.$order_info['payment_city'],
				...
				$data['payment_address'] = str_replace(array("\r\n", "\r", "\n"), '<br />', preg_replace(array("/\s\s+/", "/\r\r+/", "/\n\n+/"), '<br />', trim(str_replace($find, $replace, $format)))); //инициализация payment_address
				...
				$replace = array(
					...
					/*'date_ship' => $order_info['shipping_date_ship'],
					'time_ship' => $order_info['shipping_time_ship'],*/
				...
				if ($this->config->get('config_order_mail')) {
					...
					// HTML Mail (for manager)
                    /* my script for device definition */
                    require_once 'Mobile_Detect.php';
                    $detect = new Mobile_Detect;
                    $device_source = " Something wrong. ";
                    if ( $detect->isMobile() ) {
                        $device_source = $language->get('text_source_app');
                    } else {
                        $device_source = $language->get('text_source_shop');
                    }

					$order_to_the_city = $this->isRegionCity($order_info['payment_city']);
					$html_order_to_the_city = "";
					if ($order_to_the_city != "") {
						$html_order_to_the_city = "<h2>Заказ передан в город " . $order_to_the_city . "</h2><br>";
					}
					$data['text_greeting'] = $html_order_to_the_city . $language->get('text_new_received')." ".$device_source;
					/* end my script */
					...
					//Create order_detail
					$filename = "order_detail.xml";
					$file_order_detail = fopen($filename, "w");
					$offers = "<order>\n<items>\n";
					foreach ($order_product_query->rows as $product) {
						$offers .= "<item>\n<id>\n". $product['model'] . "\n</id>\n<count>\n" . $product['quantity'] . "\n</count>\n</item>\n";
					}
					$offers .= "</items>\n<customer>\n".$data["payment_address"]."\n</customer>\n</order>";
					$write_order_detail = fwrite($file_order_detail, $offers);
					fclose($file_order_detail);
	...
	private function isRegionCity ($inputRegion) {
		$currentRegion = "";
		$inputRegion = mb_strtolower($inputRegion);
		if(stristr($inputRegion, 'питер') || stristr($inputRegion, 'петербург') || stristr($inputRegion, 'спб')) {
			$currentRegion = "Санкт-Петербург";
		} elseif (stristr($inputRegion, 'новгород')) {
			$currentRegion = "Нижний Новгород";
		} elseif (stristr($inputRegion, 'воронеж')) {
			$currentRegion = "Воронеж";
		} elseif (stristr($inputRegion, 'казань')) {
			$currentRegion = "Казань";
		} elseif (stristr($inputRegion, 'уфа')) {
			$currentRegion = "Уфа";
		} elseif (stristr($inputRegion, 'екатеринбург') || stristr($inputRegion, 'екб')) {
			$currentRegion = "Екатеринбург";
		} elseif (stristr($inputRegion, 'челябинск')) {
			$currentRegion = "Челябинск";
		}

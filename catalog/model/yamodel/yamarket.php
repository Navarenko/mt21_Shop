...
Class ModelYamodelYamarket extends Model
{
	...
	public function getProducts($allowed_categories, $vendor_required = true) {
		$sql = "SELECT p.*, pd.name, pd.description, m.name AS manufacturer, p2c.category_id, IFNULL(ps.price, p.price) AS price, ps.price AS special, wcd.unit AS weight_unit,
	...
	function add_offer($id, $data, $available = true)
	{
		...
		foreach($allowed as $key)
			if (isset($tmp[$key]) && !empty($tmp[$key]))
				$data[$key] = $tmp[$key]; # Ïîðÿäîê âàæåí äëÿ ß.Ìàðêåòà!!!
	...
	function get_xml()
	{
		...
		$text = str_replace("vendorCode", "article", $xml);

		
		
		$import_items = new SimpleXMLElement($text);
		foreach ($import_items->shop->offers->offer as $offer) {

			/* Наличие на складе */
			$xmlstr = file_get_contents ("https://mt21.ru/shop/xml/Ostatki.xml");
			$import_stock_items = new SimpleXMLElement($xmlstr);
			$flag = false;
			$stock_status = 0;
			$model = $offer->article;
			foreach ($import_stock_items->Sklad->item as $item) {
				if ($item->ref != "") {
			...
			if ($flag || $flag_individual_items || ($model == "набор")) {
				if (($stock_status > 2) || ($model == "набор")) {
					$offer->article .= " | В наличии";
				} elseif (($stock_status <= 2) || ($flag_individual_items)) {
					$offer->article .= " | В ограниченном количестве";
				}
			}
			/* end Наличие на складе */

			$offer->article = htmlspecialchars_decode($offer->article); //форматирование наличия
			$offer->description = htmlspecialchars_decode($offer->description); //форматирование описания
			/* $temp = str_replace("><", "> <", $temp);
			$temp = strip_tags($temp); 
			$temp = str_replace("&nbsp;", " ", $temp); */ //старые способы
		}

		$fp = fopen("export_mobium_xml.xml", "w");
		fwrite($fp, $import_items->asXML());
		fclose($fp);
